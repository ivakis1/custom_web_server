...
public class JavacheConfigService {
    private static final String CONFIG_FOLDER_PATH = WebConstants
    ...
    private Set<String> requestHandlePriority;
    ...
    private void loadRequestHandlerConfig() throws IOException {
        ...
        if(!priorityConfigFile.exists() || !priorityConfigFile.isFile()){
            throw new IllegalArgumentException("Request Handler priority configuration is missing.");
        }

        String configFileContent = Reader.readAllLines(new BufferedInputStream(
                new FileInputStream(priorityConfigFile)));

        this.requestHandlePriority = Arrays.stream(configFileContent
                .replace("request-handlers:", "")
                .split(",")).collect(Collectors.toSet());
    }

    public Set<String> getRequestHandlePriority(){
        return Collections.unmodifiableSet(this.requestHandlePriority);
    }
}
